...
class AdminUserController extends Controller
{
    ...
    {
        $data = $this->admin->where('id', '<>', 1)->orderBy("created_at", "desc")->paginate(15);
    ...
    public function store(ValidateAddAdminUser $request)
    {
        try {
            ...
            $dataUploadImage = $this->storageTraitUpload($request, "avatar_path", "user");
            if (!empty($dataUploadImage)) {
                $dataAdminUserCreate["avatar_path"] = $dataUploadImage["file_path"];
            }
            //   dd($dataUserCreate);
    ...
    public function update(ValidateEditAdminUser $request, $id)
    {
        ...
        try {
            ...
            $dataUploadImage = $this->storageTraitUpload($request, "avatar_path", "user");
            if (!empty($dataUploadImage)) {
                $dataAdminUserUpdate["avatar_path"] = $dataUploadImage["file_path"];
            }
