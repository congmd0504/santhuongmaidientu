...
class User extends Authenticatable
{
     ...
     */
    protected $fillable = [
        'name', 'email', 'username', 'parent_id', 'parent_id2',
        'order', 'password', 'active', 'phone', 'date_birth',
    ...
    public function getTotalMoneyMH()
    {
        return $this->points()->select(Point::raw('SUM(point) as total'))->where("active", 1)->wherIn("points.type", config("point.listTypePointMH"));
    }
    public function getTotalMoneyRut()
    {
        return $this->points()->select(Point::raw('SUM(point) as total'))->where("active", 1)->wherIn("points.type", config("point.listTypePointRut"));
    }
    // lấy user con 20 tầng
    ...
    {
        return $this
            ->belongsToMany(Transaction::class, Hoahong::class, 'user_id', 'transaction_id')
            ->withTimestamps();
    }
