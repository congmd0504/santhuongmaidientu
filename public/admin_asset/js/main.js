$(function () {
    ...
    $(document).on('click', '.lb-status-user', function () {
        event.preventDefault();
        let wrapActive = $(this).parents('.wrap-load-active');
        let urlRequest = wrapActive.data("url");
        let value = $(this).data("value");
        ...
        let title = '';
        if (value == 2) {
            title = 'Bạn có chắc chắn muốn bỏ kyc ' + type;
        } else {
            title = 'Bạn có chắc chắn muốn KYC ' + type;
        }
        Swal.fire({
            title: title,
            icon: 'warning',
    ...
    $(document).on('click', '.btn-chot', function () {
        event.preventDefault();
        const url = $(this).attr("href");
        Swal.fire({
            title: "Bạn có chắc chắn muốn chốt doanh số",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
    ...
    // js load change pay
    $(document).on('click', '.lb-pay', function () {
        event.preventDefault();
        let wrapPay = $(this).parents('.wrap-load-pay');
        let urlRequest = wrapPay.data("url");
        let value = $(this).data("value");
        let type = $(this).data("type");
        let title = '';
        if (value) {
