      ...
      <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                ...
                @can('admin-user_frontend-upDatePointWhenEndMonth')
                <a href="{{route('admin.user_frontend.upDatePointWhenEndMonth')}}" class="btn  btn-info btn-md mb-2 btn-chot">Chốt doanh số tháng</a>
                @endcan
                @can('admin-user_frontend-upDatePointWhenEndYear')
                <a href="{{route('admin.user_frontend.upDatePointWhenEndYear')}}" class="btn  btn-info btn-md mb-2 btn-chot">Chốt doanh số năm</a>
                @endcan
                <form class="form-inline ml-3" action="{{route('admin.user_frontend.export.excel.database')}}" method="post" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="d-inline-block">
                        ...
                        <label for="">Chọn loại user</label>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="form-group col-md-3 mb-0" style="min-width:100px;">
                                                <select id="" name="fill_status" class="form-control">
                                                    <option value="">-- Lọc --</option>
                                                    <option value="0" {{ $fill_status=='0'? 'selected':'' }}>Chưa gửi yêu cầu kyc</option>
                                                    <option value="1" {{ $fill_status=='1'? 'selected':'' }}>Đã Gửi yêu cầu KYC</option>
                                                    <option value="2" {{ $fill_status=='2'? 'selected':'' }}>Đã kích hoạt</option>
                                                </select>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <table class="table table-head-fixed" style="font-size:13px;">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="wrap-load-active" data-url="{{ route('admin.user_frontend.load.status',['id'=>$item->id]) }}">
                                        @include('admin.components.load-change-status-user',['data'=>$item,'type'=>'user'])
                                     </td>
                                    <td>
                                      ...
                                      @endif --}}
                                        ...
                                        <a  class="btn btn-sm btn-success"  href="{{route('admin.user_frontend.napTien',['id'=>$item->id])}}" ><i class="fas fa-money-check-alt"></i> Nạp tiền</a>
                                        <a  class="btn btn-sm btn-info popup_edit" data-id="{{ $item->id }}" href="javascript:void(0);" ><i class="fas fa-level-up-alt"></i> Nâng cấp</a>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
...
<!-- Modal Nâng cấp level-->
<div class="modal fade" id="editWarehouseModal" tabindex="-1" role="dialog" aria-labelledby="editWarehouseModal" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editWarehouseModalLabel">Nâng cấp tài khoản</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body" id="showEditWarehouse">
...
<script>
    $(document).ready(function(){
        ...
        $('.popup_edit').on('click', function(){
            ...
            $.ajax({
                url: '{{ route('admin.user_frontend.level') }}',
                type: 'GET',
                data: {
                    id: dataId,
                },
                success: function(response) {
                    $('#showEditWarehouse').html(response);
                },
